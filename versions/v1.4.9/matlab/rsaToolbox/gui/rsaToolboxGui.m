...
% Last Modified by GUIDE v2.5 03-Feb-2014 10:31:56
...
event_listString=[event_listString;{'rsaToolbox v1.4.9'}];
	...
	if isempty(dataExp)
		...
		if isempty(intercept) | isnan(intercept)~=0
			...
			if isempty(B) | isnan(B)~=0
				...
				[experimentalRVPre,experimentalRVPreCutoff,experimentalRVDif,experimentalRVDifCutoff,experimentalTTOTFCutoff,experimentalRVDifMean,experimentalRVDifCutoffMean]=rsaApplyCal(experimentalTTOTF,experimentalRV,B,intercept,10);
				axes(handles.output_rsa);
	...
	% Otherwise construct the fullfilename and Check and load the file.
		...
		if ~isempty(str2num(save_intercept)) & ~isempty(str2num(save_B))
			...
			if (fid < 0)
				...
				fprintf(fid,'%s\n',char('rsaToolbox v1.4.9 by Stefan M. Schulz <schulz@psychologie.uni-wuerzburg.de> - regression parameters'));
				global infantOption;
				if infantOption==1
...
% handles	structure with handles and user data (see GUIDATA)
	...
	if isequal([fileName,pathName],[0,0])
		...
		if (fid < 0)
			...
			event_listString=[event_listString;{'______________________________________________________________________'}];
			fprintf(fid,'%s\n',char(['rsaToolbox v1.4.9 by Stefan M. Schulz <schulz@psychologie.uni-wuerzburg.de> - event monitor output']));
			global infantOption;
			if infantOption==1
				fprintf(fid,'%s\n',char(['Note: infant option is',char(9),'on']));
			else
...
% handles	structure with handles and user data (see GUIDATA)
	...
	if sTemp(1,2)>0
		...
		if (fid < 0)
			...
			fprintf(fid,'%s\n',char('rsaToolbox v1.4.9 by Stefan M. Schulz <schulz@psychologie.uni-wuerzburg.de> - RSA data output - one column per data set'));
			global infantOption;
			if infantOption==1
				fprintf(fid,'%s\n',char(['Note: infant option is',char(9),'on']));
			else
...
% handles	structure with handles and user data (see GUIDATA)
	...
	if sTemp(1,2)>0
		...
		if (fid < 0)
			...
			fprintf(fid,'%s\n',char('rsaToolbox v1.4.9 by Stefan M. Schulz <schulz@psychologie.uni-wuerzburg.de> - RSA/VT data output - one column per data set'));
			global infantOption;
			if infantOption==1
				 fprintf(fid,'%s\n',char(['Note: infant option is',char(9),'on']));
			else
...
% handles	structure with handles and user data (see GUIDATA)
	...
	if sTemp(1,2)>0
		...
		if (fid < 0)
			...
			fprintf(fid,'%s\n',char('rsaToolbox v1.4.9 by Stefan M. Schulz <schulz@psychologie.uni-wuerzburg.de> - TTOT-corrected RSA/VT data output - one column per data set'));
			global infantOption;
			if infantOption==1
				   fprintf(fid,'%s\n',char(['Note: infant option is',char(9),'on']));
			else
...
% handles	structure with handles and user data (see GUIDATA)
	...
	if cancelFlag==0
		...
		for j=1:d
			...
			if j == 1
				fprintf(fid,'%s\n',char(['rsaToolbox v1.4.9 by Stefan M. Schulz <schulz@psychologie.uni-wuerzburg.de> - Analysis:',char(9),datestr(now)]));
			...
			if j == 1
				fprintf(fid,'%s\n',char(['rsaToolbox v1.4.9 by Stefan M. Schulz <schulz@psychologie.uni-wuerzburg.de> - Analysis: ',datestr(now)]));
...
% handles	structure with handles and user data (see GUIDATA)
	...
	if cancelFlag==0
		...
		fprintf(fid,'%s\n',char('rsaToolbox v1.4.9 by Stefan M. Schulz <schulz@psychologie.uni-wuerzburg.de> - paced breathing RSA data output - one column per paced breathing data set'));
		global infantOption;
		if infantOption==1
			fprintf(fid,'%s\n',char(['Note: infant option is',char(9),'on']));
		else
		...
		fprintf(fid,'%s\n',char('rsaToolbox v1.4.9 by Stefan M. Schulz <schulz@psychologie.uni-wuerzburg.de> - paced breathing RSA/VT data output - one column per paced breathing data set'));
		global infantOption;
		if infantOption==1
			fprintf(fid,'%s\n',char(['Note: infant option is',char(9),'on']));
		else
...
% --------------------------------------------------------------------
function vivologicImport_Callback(hObject, eventdata, handles)
% hObject	handle to vivologicImport (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles	structure with handles and user data (see GUIDATA)
	event_listString=get(handles.event_list,'String');
	event_listString=[event_listString;{['Import filter for VivioMetrics',char(174),' VivoLogic',char(174),' data export. - Copyright ',char(169),' 2009-2013 by Stefan M. Schulz']}];
	event_listString=[event_listString;{'______________________________________________________________________'}];
	event_listString=[event_listString;{'Quick Reference:'}];
	...
	if isequal([fileName,pathName],[0,0])
		...
		event_listString=[event_listString;{['VivoLogic',char(174),' data import aborts.']}];
		event_listString=[event_listString;{'______________________________________________________________________'}];
		set(handles.event_list,'String',event_listString);
		sTemp=size(event_listString);
		set(handles.event_list,'Value',sTemp(1,1));
		...
		for i=1:length(dateienResp)
			...
			if length(filePrefixResp)==length(filePrefixRR) & filePrefixResp==filePrefixRR
				...
				event_listString=[event_listString;{['VivoLogic',char(174),' data import aborts - please check your data!']}];
				event_listString=[event_listString;{'______________________________________________________________________'}];
				set(handles.event_list,'String',event_listString);
				sTemp=size(event_listString);
				set(handles.event_list,'Value',sTemp(1,1));
		...
		for j=1:length(batchList)
			...
			if j==1
				...
				fprintf(fid,'%s\n',char(['rsaToolbox v1.4.9 by Stefan M. Schulz <schulz@psychologie.uni-wuerzburg.de> - VivoLogic',char(174),' data import:',char(9),datestr(now)]));
				fprintf(fid,'%s\t','data folder');
				if noOfFilenameElements>0
					fprintf(fid,'%s\t','dataset');
					for nFilenameElements=1:noOfFilenameElements+1
...
%				fprintf(fid,'%s\t',num2str(length(find(rrDataFlag3SD==2)))-(length(find(rrDataFlag==1))-1));
	...
	event_listString=[event_listString;{' '}];
	event_listString=[event_listString;{['Import filter for VivoLogic',char(174),' data export - terminated succesfully.']}];
	event_listString=[event_listString;{'______________________________________________________________________'}];
	set(handles.event_list,'String',event_listString);
	sTemp=size(event_listString);
	set(handles.event_list,'Value',sTemp(1,1));
...
% handles	structure with handles and user data (see GUIDATA)
	event_listString=get(handles.event_list,'String');
	event_listString=[event_listString;{['Import filter for Vivonoetics',char(174),' VivoSense',char(174),' data export. - Copyright ',char(169),' 2013 by Stefan M. Schulz']}];
	event_listString=[event_listString;{'______________________________________________________________________'}];
	event_listString=[event_listString;{'Quick Reference:'}];
	event_listString=[event_listString;{'Two exports are required to use this tool:'}];
	event_listString=[event_listString;{'The EKG R-peak (RR) export file has to include the following columns:'}];
	...
	if isequal([fileName,pathName],[0,0])
		...
		event_listString=get(handles.event_list,'String');
		event_listString=[event_listString;{'Please, select a valid file.'}];
		event_listString=[event_listString;{['VivoSense',char(174),' data import aborts.']}];
		event_listString=[event_listString;{'______________________________________________________________________'}];
		set(handles.event_list,'String',event_listString);
		sTemp=size(event_listString);
		set(handles.event_list,'Value',sTemp(1,1));
		...
		for i=1:length(dateienResp)
			...
			if length(filePrefixResp)==length(filePrefixRR) & filePrefixResp==filePrefixRR
				...
				event_listString=get(handles.event_list,'String');
				event_listString=[event_listString;{'Inconsistencies in batch file list found.'}];
				event_listString=[event_listString;{['VivoSense',char(174),' data import aborts. - please check your data!']}];
				event_listString=[event_listString;{'______________________________________________________________________'}];
				set(handles.event_list,'String',event_listString);
				sTemp=size(event_listString);
				set(handles.event_list,'Value',sTemp(1,1));
		...
		for j=1:length(batchList)
			...
			if j==1
				fid = fopen([pathName,'data_generated_for_rsaToolbox\description.txt'],'a');
				fprintf(fid,'%s\n',char(['rsaToolbox v1.4.9 by Stefan M. Schulz <schulz@psychologie.uni-wuerzburg.de> - VivoSense',char(174),' data import:',char(9),datestr(now)]));
				fprintf(fid,'%s\t','data folder');
				if noOfFilenameElements>0
					fprintf(fid,'%s\t','dataset');
					for nFilenameElements=1:noOfFilenameElements+1
...
%				fprintf(fid,'%s\t',num2str(length(find(rrDataFlag3SD==2)))-(length(find(rrDataFlag==1))-1));
	...
	event_listString=[event_listString;{'to allow rsaToolbox to exhaust the given information.'}];
	event_listString=[event_listString;{' '}];
	event_listString=[event_listString;{['Import filter for VivoSense',char(174),' data export - terminated succesfully.']}];
	event_listString=[event_listString;{'______________________________________________________________________'}];
	set(handles.event_list,'String',event_listString);
	sTemp=size(event_listString);
	set(handles.event_list,'Value',sTemp(1,1));
