...
event_listString=[event_listString;{'rsaToolbox v2.0.2'}];
	...
	% Otherwise construct the fullfilename and Check and load the file.
		...
		if ~isempty(str2num(save_intercept)) & ~isempty(str2num(save_B))
			...
			if (fid < 0)
				...
				fprintf(fid,'%s\n',char('rsaToolbox v2.0.2 by Stefan M. Schulz <schulz@psychologie.uni-wuerzburg.de> - regression parameters'));
				global infantOption;
				if infantOption==1
...
% handles	structure with handles and user data (see GUIDATA)
	...
	if isequal([fileName,pathName],[0,0])
		...
		if (fid < 0)
			...
			event_listString=[event_listString;{'______________________________________________________________________'}];
			fprintf(fid,'%s\n',char(['rsaToolbox v2.0.2 by Stefan M. Schulz <schulz@psychologie.uni-wuerzburg.de> - event monitor output']));
			global infantOption;
			if infantOption==1
				fprintf(fid,'%s\n',char(['Note: infant option is',char(9),'on']));
			else
...
% handles	structure with handles and user data (see GUIDATA)
	...
	if sTemp(1,2)>0
		...
		if (fid < 0)
			...
			fprintf(fid,'%s\n',char('rsaToolbox v2.0.2 by Stefan M. Schulz <schulz@psychologie.uni-wuerzburg.de> - RSA data output - one column per data set'));
			global infantOption;
			if infantOption==1
				fprintf(fid,'%s\n',char(['Note: infant option is',char(9),'on']));
			else
...
% handles	structure with handles and user data (see GUIDATA)
	...
	if sTemp(1,2)>0
		...
		if (fid < 0)
			...
			fprintf(fid,'%s\n',char('rsaToolbox v2.0.2 by Stefan M. Schulz <schulz@psychologie.uni-wuerzburg.de> - RSA/VT data output - one column per data set'));
			global infantOption;
			if infantOption==1
				 fprintf(fid,'%s\n',char(['Note: infant option is',char(9),'on']));
			else
...
% handles	structure with handles and user data (see GUIDATA)
	...
	if sTemp(1,2)>0
		...
		if (fid < 0)
			...
			fprintf(fid,'%s\n',char('rsaToolbox v2.0.2 by Stefan M. Schulz <schulz@psychologie.uni-wuerzburg.de> - TTOT-corrected RSA/VT data output - one column per data set'));
			global infantOption;
			if infantOption==1
				   fprintf(fid,'%s\n',char(['Note: infant option is',char(9),'on']));
			else
...
% handles	structure with handles and user data (see GUIDATA)
	...
	if cancelFlag==0
		...
		for j=1:d
			...
			if j == 1
				fprintf(fid,'%s\n',char(['rsaToolbox v2.0.2 by Stefan M. Schulz <schulz@psychologie.uni-wuerzburg.de> - Analysis:',char(9),datestr(now)]));
                fprintf(fid,'%s\n',char(['data folder:',char(9),batchDirExp]));
                if splitFilename==0
                    fprintf(fid,'%s\t',char(['filename']));
                else
                    for nFilenameElements=1:noOfFilenameElements+1
                        fprintf(fid,'%s\t',['filename_part',num2str(nFilenameElements)]);
                    end;
                end;
                fprintf(fid,'%s\t',char(['set-index']));
                fprintf(fid,'%s\t',char(['line-index']));
                fprintf(fid,'%s\t','PVC');
				fprintf(fid,'%s\t','infant option');
            ...
            setIndex=[1:1:numel(tempDataOutput(:,1))];
            setIndex=setIndex'*0+j;
            lineIndex=[1:1:numel(tempDataOutput(:,1))];
            lineIndex=lineIndex';
            pvcPrintout=[1:1:numel(tempDataOutput(:,1))];
            pvcPrintout=pvcPrintout'*0+PVC;
			global infantOption;
            infantOptionPrintout=[1:1:numel(tempDataOutput(:,1))];
            infantOptionPrintout=infantOptionPrintout'*0+infantOption;
            tempDataOutput=[setIndex,lineIndex,pvcPrintout,infantOptionPrintout,tempDataOutput];
            for c=1:numel(tempDataOutput(:,1))
                if splitFilename==0
                    datasetPrintout{c,1}=tmpBatchFileSavename;
                else
                    datasetPrintout{c,1}=tmpBatchFileSavenamePrint;
                end;
            end;
            for x=1:length(IBIdiffExpF)
				fprintf(fid,'%s\t',datasetPrintout{x,1});
			...
			if j == 1
				fprintf(fid,'%s\n',char(['rsaToolbox v2.0.2 by Stefan M. Schulz <schulz@psychologie.uni-wuerzburg.de> - Analysis: ',datestr(now)]));
...
%			   fprintf(fid,'%s\t','max RSA [ms] - all breaths');
				...
				fprintf(fid,'%s\t','min IBI [ms] - meeting quality criteria');
				fprintf(fid,'%s\t','max IBI [ms] - meeting quality criteria');
				...
				fprintf(fid,'%s\t','min IBI [ms] - not enough IBI');
				fprintf(fid,'%s\t','max IBI [ms] - not enough IBI');
				...
				fprintf(fid,'%s\t','min IBI [ms] - failed applied PVC');
				fprintf(fid,'%s\t','max IBI [ms] - failed applied PVC');
...
% handles	structure with handles and user data (see GUIDATA)
	...
	if cancelFlag==0
		...
		fprintf(fid,'%s\n',char('rsaToolbox v2.0.2 by Stefan M. Schulz <schulz@psychologie.uni-wuerzburg.de> - paced breathing RSA data output - one column per paced breathing data set'));
		global infantOption;
		if infantOption==1
			fprintf(fid,'%s\n',char(['Note: infant option is',char(9),'on']));
		else
		...
		fprintf(fid,'%s\n',char('rsaToolbox v2.0.2 by Stefan M. Schulz <schulz@psychologie.uni-wuerzburg.de> - paced breathing RSA/VT data output - one column per paced breathing data set'));
		global infantOption;
		if infantOption==1
			fprintf(fid,'%s\n',char(['Note: infant option is',char(9),'on']));
		else
		...
		if isempty(B) | isnan(B)~=0
			...
			if cancelFlag==0
				...
				for j=1:d
                    ...
                    global infantOption;
                    if PVC==5
						tempDataOutput=[IBIdiffExpF,experimentalRV,experimentalRVExpDifNan,experimentalTTOTF,experimentalVTF,HRperBreathExpF,expFlag,IBInoExpF];
                        setIndex=[1:1:numel(tempDataOutput(:,1))];
                        setIndex=setIndex'*0+j;
                        lineIndex=[1:1:numel(tempDataOutput(:,1))];
                        lineIndex=lineIndex';
						...
						tempDataOutput=[IBIdiffExpF,experimentalRV,experimentalRVDif,experimentalTTOTF,experimentalVTF,HRperBreathExpF,expFlag,IBInoExpF];
                        setIndex=[1:1:numel(tempDataOutput(:,1))];
                        setIndex=setIndex'*0+j;
                        lineIndex=[1:1:numel(tempDataOutput(:,1))];
                        lineIndex=lineIndex';
                        tempDataOutput=[setIndex,lineIndex,tempDataOutput];
                        pvcPrintout=[1:1:numel(tempDataOutput(:,1))];
                        pvcPrintout=pvcPrintout'*0+PVC;
                        infantOptionPrintout=[1:1:numel(tempDataOutput(:,1))];
                        infantOptionPrintout=infantOptionPrintout'*0+infantOption;
	...
	if isequal([fileName,pathName],[0,0])
		...
		for j=1:length(batchList)
			...
			if j==1
				...
				fprintf(fid,'%s\n',char(['rsaToolbox v2.0.2 by Stefan M. Schulz <schulz@psychologie.uni-wuerzburg.de> - VivoLogic',char(174),' data import v01:',char(9),datestr(now)]));
				fprintf(fid,'%s\t','data folder');
				if noOfFilenameElements>0
					fprintf(fid,'%s\t','dataset');
					for nFilenameElements=1:noOfFilenameElements+1
    			...
    			fprintf(fid,'%s',char(['rsaToolbox v2.0.2 by Stefan M. Schulz <schulz@psychologie.uni-wuerzburg.de> - VivoLogic',char(174),' data import v01:',char(9),datestr(now)]));
				fprintf(fid,'\n');
				fclose(fid);
            end;
			...
			n=1;
			while (rrTime(n,1)<respTime(1,1)) & (n<length(rrTime(:,1)))
				n=n+1;
			end;
            if n>length(rrTime(:,1))
        		event_listString=get(handles.event_list,'String');
                event_listString=[event_listString;{['WARNING!!! Data in ',filename01,' and ',filename02,' does not overlap - please check your data exports.']}];
            	set(handles.event_list,'String',event_listString);
                sTemp=size(event_listString);
                set(handles.event_list,'Value',sTemp(1,1));
                guidata(hObject,handles);
       			fid = fopen([pathName,'data_generated_for_rsaToolbox\errorLog.txt'],'a');
                fprintf(fid,'%s','WARNING!!! Data in ',filename01,' and ',filename02,' does not overlap');
    			fprintf(fid,'\n');
                fclose(fid);
                return
            else
...
% handles	structure with handles and user data (see GUIDATA)
	...
	if isequal([fileName,pathName],[0,0])
		...
		for j=1:length(batchList)
			...
			if j==1
				fid = fopen([pathName,'data_generated_for_rsaToolbox\description.txt'],'a');
				fprintf(fid,'%s\n',char(['rsaToolbox v2.0.2 by Stefan M. Schulz <schulz@psychologie.uni-wuerzburg.de> - VivoLogic',char(174),' data import v02:',char(9),datestr(now)]));
				fprintf(fid,'%s\t','data folder');
				if noOfFilenameElements>0
					fprintf(fid,'%s\t','dataset');
					for nFilenameElements=1:noOfFilenameElements+1
...
% handles	structure with handles and user data (see GUIDATA)
	...
	if isequal([fileName,pathName],[0,0])
		...
		for j=1:length(batchList)
			...
			if j==1
				fid = fopen([pathName,'data_generated_for_rsaToolbox\description.txt'],'a');
				fprintf(fid,'%s\n',char(['rsaToolbox v2.0.2 by Stefan M. Schulz <schulz@psychologie.uni-wuerzburg.de> - VivoSense',char(174),' data import v01:',char(9),datestr(now)]));
				fprintf(fid,'%s\t','data folder');
				if noOfFilenameElements>0
					fprintf(fid,'%s\t','dataset');
					for nFilenameElements=1:noOfFilenameElements+1
				...
				fclose(fid);
      			fid = fopen([pathName,'data_generated_for_rsaToolbox\errorLog.txt'],'a');
    			fprintf(fid,'%s',char(['rsaToolbox v2.0.2 by Stefan M. Schulz <schulz@psychologie.uni-wuerzburg.de> - VivoSense',char(174),' data import v01:',char(9),datestr(now)]));
				fprintf(fid,'\n');
				fclose(fid);
			end;
...
% handles	structure with handles and user data (see GUIDATA)
	...
	if isequal([fileName,pathName],[0,0])
		...
		for j=1:length(batchList)
			...
			if j==1
				fid = fopen([pathName,'data_generated_for_rsaToolbox\description.txt'],'a');
				fprintf(fid,'%s\n',char(['rsaToolbox v2.0.2 by Stefan M. Schulz <schulz@psychologie.uni-wuerzburg.de> - VivoSense',char(174),' data import v02:',char(9),datestr(now)]));
				fprintf(fid,'%s\t','data folder');
				if noOfFilenameElements>0
					fprintf(fid,'%s\t','dataset');
					for nFilenameElements=1:noOfFilenameElements+1
				...
				fclose(fid);
      			fid = fopen([pathName,'data_generated_for_rsaToolbox\errorLog.txt'],'a');
    			fprintf(fid,'%s',char(['rsaToolbox v2.0.2 by Stefan M. Schulz <schulz@psychologie.uni-wuerzburg.de> - VivoSense',char(174),' data import v02:',char(9),datestr(now)]));
				fprintf(fid,'\n');
				fclose(fid);
			end;
